...
// Con comparación Local vs Hardware Real
// ===============================================
...
let serialResponseHandlers = []; // Handlers para capturar respuestas específicas del Arduino
            ...
            while (true) {
                ...
                lines.forEach(line => {
                    if (line.trim()) {
                        ...
                        // NUEVO: Procesar con handlers registrados
                        serialResponseHandlers.forEach(handler => {
                            try {
                                handler(line.trim());
                            } catch (e) {
...
async function sendAnalysis() {
    ...
    addToConsole('✅ Análisis enviado al Arduino', 'success');
    addToConsole('⏳ Esperando distribución a esclavos I2C...', 'warning');
    
    // Esperar a que Arduino distribuya (2 segundos)
    await sleep(2000);
    
    // LEER ESTADOS REALES DE LOS ESCLAVOS
    addToConsole('📡 Leyendo estados reales de Quantum32...', 'warning');
    
    try {
        const realStates = await readRealSlaveStates();
        
        if (realStates && realStates.length === 4) {
            addToConsole('🔬 Estados leídos de Quantum32 Real:', 'success');
            realStates.forEach((state, idx) => {
                const diff = Math.abs(state - q32.boundary_states[idx]);
                const diffStr = diff === 0 ? '✅' : diff < 5 ? `⚠️ Δ${diff}` : `❌ Δ${diff}`;
                addToConsole(`  Esclavo ${idx}: ${state} (local: ${q32.boundary_states[idx]}) ${diffStr}`);
            });
            
            // Guardar estados reales en el análisis
            currentAnalysis.quantum32_real = {
                boundary_states: realStates,
                source: 'QUANTUM32_HARDWARE',
                timestamp: Date.now()
            };
            
            // Actualizar visualización con comparación
            displayQuantum32States(currentAnalysis);
            
            showNotification('✅ Datos sincronizados con Quantum32 Real', 'success');
        } else {
            addToConsole('⚠️ No se pudieron leer todos los esclavos', 'warning');
            showNotification('Análisis enviado (sin verificación)', 'warning');
        }
    } catch (error) {
        addToConsole(`⚠️ Error leyendo esclavos: ${error.message}`, 'warning');
        showNotification('Análisis enviado (sin verificación)', 'warning');
    }
}

async function readRealSlaveStates() {
    return new Promise(async (resolve, reject) => {
        const timeout = setTimeout(() => {
            reject(new Error('Timeout esperando respuesta de esclavos'));
        }, 5000);
        
        const states = [];
        let linesReceived = 0;
        
        // Listener temporal para capturar respuesta
        const responseHandler = (line) => {
            // Parsear: "  Esclavo 0 (0x10): 145"
            const match = line.match(/Esclavo\s+(\d+)\s+\(0x[\dA-F]+\):\s+(\d+)/i);
            if (match) {
                ...
                // Si tenemos los 4 esclavos, resolver
                if (linesReceived >= 4) {
                    clearTimeout(timeout);
                    // Remover handler
                    const handlerIdx = serialResponseHandlers.indexOf(responseHandler);
                    if (handlerIdx > -1) {
                        serialResponseHandlers.splice(handlerIdx, 1);
                    }
                    resolve(states);
                }
            }
        ...
        // Registrar handler
        serialResponseHandlers.push(responseHandler);
        
        // Enviar comando READ_SLAVES
        await sendCmd('READ_SLAVES');
    });
}
...
function displayQuantum32States(data, container) {
    const q32 = data.quantum32_data;
    const q32Real = data.quantum32_real;
    ...
    html += '<strong>⚛️ Estados Quantum32</strong>';
    
    if (q32Real && q32Real.source === 'QUANTUM32_HARDWARE') {
        html += '<span style="background: #00af89; color: white; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: bold;">🔬 HARDWARE REAL</span>';
    } else {
        html += '<span style="background: #fc3; color: #202122; padding: 4px 12px; border-radius: 12px; font-size: 12px; font-weight: bold;">💻 LOCAL</span>';
    }
    html += '</div>';

    // Tabla comparativa si hay datos reales
    if (q32Real && q32Real.boundary_states) {
        const categorias = ['👥 Entidades', '⚡ Acciones', '💡 Conceptos', '🔧 Propiedades'];
        
        html += '<table style="width: 100%; margin-bottom: 20px; border-collapse: collapse;">';
        html += '<thead>';
        html += '<tr style="background: #f8f9fa; border-bottom: 2px solid #a2a9b1;">';
        html += '<th style="padding: 8px; text-align: left;">Esclavo</th>';
        html += '<th style="padding: 8px; text-align: center;">💻 Local</th>';
        html += '<th style="padding: 8px; text-align: center;">🔬 Quantum32</th>';
        html += '<th style="padding: 8px; text-align: center;">Δ</th>';
        html += '</tr>';
        html += '</thead>';
        html += '<tbody>';
        ...
        for (let i = 0; i < 4; i++) {
            const local = q32.boundary_states[i];
            const real = q32Real.boundary_states[i];
            const diff = Math.abs(real - local);
            const diffPercent = ((diff / 255) * 100).toFixed(2);
            
            totalDiff += diff;
            
            const color = diff === 0 ? '#00af89' : diff < 5 ? '#fc3' : '#d33';
            const icon = diff === 0 ? '✅' : diff < 5 ? '⚠️' : '❌';
            
            html += '<tr style="border-bottom: 1px solid #eaecf0;">';
            html += `<td style="padding: 8px;"><strong>${categorias[i]}</strong></td>`;
            html += `<td style="padding: 8px; text-align: center;">${local}</td>`;
            html += `<td style="padding: 8px; text-align: center; color: ${color}; font-weight: bold;">${real}</td>`;
            html += `<td style="padding: 8px; text-align: center;">${icon} ${diff} (${diffPercent}%)</td>`;
            html += '</tr>';
        }
        ...
        const avgDiff = (totalDiff / 4).toFixed(2);
        const avgPercent = ((totalDiff / (4 * 255)) * 100).toFixed(2);
        
        let statusColor, statusIcon, statusText;
        if (totalDiff === 0) {
            statusColor = '#00af89';
            statusIcon = '✅';
            statusText = 'Perfecto sincronismo entre cálculo local y hardware';
        } else if (totalDiff < 10) {
            statusColor = '#fc3';
            statusIcon = '⚠️';
            statusText = 'Pequeñas diferencias (normal por ruido analógico del hardware)';
        } else {
            statusColor = '#d33';
            statusIcon = '❌';
            statusText = 'Diferencias significativas (revisar conexiones I2C)';
        }
        
        html += `<div style="background: ${statusColor}22; border: 1px solid ${statusColor}; padding: 12px; border-radius: 2px; margin-bottom: 15px;">`;
        html += `<strong style="color: ${statusColor};">${statusIcon} ${statusText}</strong><br>`;
        html += '<div style="margin-top: 8px; font-size: 13px; color: #54595d;">';
        html += `• Diferencia promedio: <strong>${avgDiff}</strong> unidades (<strong>${avgPercent}%</strong>)<br>`;
        html += `• Diferencia total: <strong>${totalDiff}</strong> unidades<br>`;
        html += `• Timestamp: ${new Date(q32Real.timestamp).toLocaleTimeString()}`;
        html += '</div>';
        html += '</div>';
    } else {
        // Solo mostrar estados locales
        ...
        q32.boundary_states.forEach((state, i) => {
            ...
            html += `
                <div class="boundary-state">
                    <h4>${categorias[i]} (0x${(0x10 + i).toString(16).toUpperCase()})</h4>
